...
public interface ClientsApi {
    ...
    default ResponseEntity<List<Client>> getAllClients(
        @Parameter(name = "filter_by", description = "Filters search results by client_id or name", schema = @Schema(description = "")) @Valid @RequestParam(value = "filter_by", required = false) String filterBy
    ) {
    ...
    @Operation(
        ...
        tags = { "clients" },
        responses = {
            @ApiResponse(responseCode = "200", description = "Successful operation", content = @Content(mediaType = "application/json", schema = @Schema(implementation =  ClientImportResponse.class))),
            @ApiResponse(responseCode = "400", description = "Invalid input")
        },
        security = {
            @SecurityRequirement(name = "cookieAuth")
    ...
    default ResponseEntity<ClientImportResponse> importClients(
        @Parameter(name = "file", description = "", schema = @Schema(description = "")) @RequestPart(value = "file", required = false) MultipartFile file
    ) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
